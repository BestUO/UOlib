#pragma once

#include <cstdint>
#include <iostream>
#include <fstream>
#include <string>
#include <memory>
#include <qdebug.h>
#include <time.h>
using namespace std;

#include <mutex>

#define FOURCC2 uint32_t	

#define MAKE_FOURCC2(a,b,c,d) \
( ((uint32_t)d) | ( ((uint32_t)c) << 8 ) | ( ((uint32_t)b) << 16 ) | ( ((uint32_t)a) << 24 ) )

template <char ch0, char ch1, char ch2, char ch3> struct MakeFOURCC2{ enum { value = (ch0 << 0) + (ch1 << 8) + (ch2 << 16) + (ch3 << 24) }; };


// Format chunk data field
struct Wave_format{

	uint16_t format_tag;      // WAVE的数据格式，PCM数据该值为1
	uint16_t channels;        // 声道数
	uint32_t sample_per_sec;  // 采样率
	uint32_t bytes_per_sec;   // 码率，channels * sample_per_sec * bits_per_sample / 8
	uint16_t block_align;     // 音频数据块，每次采样处理的数据大小，channels * bits_per_sample / 8
	uint16_t bits_per_sample; // 量化位数，8、16、32等
	uint16_t ex_size;         // 扩展块的大小，附加块的大小

	Wave_format()
	{
		format_tag      = 1; // PCM format data
		ex_size         = 0; // don't use extesion field

		channels        = 0;
		sample_per_sec  = 0;
		bytes_per_sec   = 0;
		block_align     = 0;
		bits_per_sample = 0;
	}

	Wave_format(uint16_t nb_channel, uint32_t sample_rate, uint16_t sample_bits)
		:channels(nb_channel), sample_per_sec(sample_rate), bits_per_sample(sample_bits)
	{
		format_tag    = 0x01;                                           // PCM format data
		bytes_per_sec = channels * sample_per_sec * bits_per_sample / 8; // 码率
		block_align   = channels * bits_per_sample / 8;
		ex_size       = 0;                                               // don't use extension field
	}
};

// The basic chunk of RIFF file format
struct Base_chunk{

	FOURCC2 fcc;    // FOURCC2 id
	uint32_t cb_size; // 数据域的大小

	Base_chunk(FOURCC2 FOURCC2)
		: fcc(FOURCC2)
	{
		cb_size = 0;
	}
};

/*

	数据格式为PCM的WAV文件的基本结构
	--------------------------------
	| Base_chunk | RIFF	|
	---------------------
	|	WAVE            |
	---------------------
	| Base_chunk | fmt  |	Header
	---------------------
	| Wave_format|      | 
	---------------------
	| Base_chunk | data |
	---------------------------------
	|    PCM data                   |
	---------------------------------
*/

/*

	数据格式为PCM的WAV文件头
	--------------------------------
	| Base_chunk | RIFF	|
	---------------------
	|	WAVE            |
	---------------------
	| Base_chunk | fmt  |	Header
	---------------------
	| Wave_format|      |
	---------------------
	| Base_chunk | data |
	--------------------------------
*/

struct Wave_header{

	shared_ptr<Base_chunk> riff;
	FOURCC2 wave_fcc;
	shared_ptr<Base_chunk> fmt;
	shared_ptr<Wave_format>  fmt_data;
	shared_ptr<Base_chunk> data;

	Wave_header(uint16_t nb_channel, uint32_t sample_rate, uint16_t sample_bits)
	{
		riff      = make_shared<Base_chunk>(MakeFOURCC2<'R', 'I', 'F', 'F'>::value);
		fmt       = make_shared<Base_chunk>(MakeFOURCC2<'f', 'm', 't', ' '>::value);
		fmt->cb_size = 18;

		fmt_data  = make_shared<Wave_format>(nb_channel, sample_rate, sample_bits);
		data      = make_shared<Base_chunk>(MakeFOURCC2<'d', 'a', 't', 'a'>::value);

		wave_fcc = MakeFOURCC2<'W', 'A', 'V', 'E'>::value;
	}

	Wave_header()
	{
		riff         = nullptr;
		fmt          = nullptr;

		fmt_data     = nullptr;
		data         = nullptr;

		wave_fcc     = 0;
	}
};

class CWaveFile
{

public:

	CWaveFile()
	{
		header = nullptr;
		data = nullptr;
	}

	// Write wav file
	bool static write(const string& filename, void *data, uint32_t length)
	{
		if (checkExistence(filename.c_str()))
		{
			fstream ofs(filename, ios::binary | ios::out | ios::in);
			if (!ofs)
				return false;

			ofs.seekp(sizeof(Base_chunk) * 2 + 26, ios::beg);
			uint32_t len;
			ofs.read((char*)&len, 4);
			ofs.seekg(-4, ios::cur);
			len += length;
			ofs.write((char*)&len,4);
			ofs.seekg(0, ios::end);
			// Write data
			ofs.write((char*)data, length);

			ofs.close();
		}
		else
		{
			ofstream ofs;
			try
			{
				ofs.open(filename, ios::binary);
			}
			catch (const std::exception& e)
			{
				std::cout << e.what();
				return false;
			}
			Wave_header header(1, 12500, 16);
			// Calculate size of RIFF chunk data
			header.data->cb_size = ((length + 1) / 2) * 2;
			header.riff->cb_size = 4 + 4 + header.fmt->cb_size + 4 + 4 + header.data->cb_size + 4;

			// Write header

			// Write RIFF
			char *chunk = (char*)header.riff.get();
			ofs.write(chunk, sizeof(Base_chunk));

			// Write WAVE FOURCC2
			ofs.write((char*)&(header.wave_fcc), 4);

			// Write fmt
			chunk = (char*)header.fmt.get();
			ofs.write(chunk, sizeof(Base_chunk));

			// Write fmt_data
			chunk = (char*)header.fmt_data.get();
			ofs.write(chunk, header.fmt->cb_size);

			// Write data
			chunk = (char*)header.data.get();
			ofs.write(chunk, sizeof(Base_chunk));

			// Write data
			ofs.write((char*)data, length);

			ofs.close();
		}
		return true;
	}

	bool read(const string &filename)
	{
		if (!read_header(filename))
			return false;

		// PCM 数据相对文件头位置的偏移量， +8（RIFF FOURCC2 +4，size + 4）
		uint32_t offset = header->riff->cb_size - header->data->cb_size + 8; 
		data = unique_ptr<uint8_t[]>(new uint8_t[header->data->cb_size]);

		ifstream ifs(filename, ifstream::binary);
		if (!ifs)
			return false;

		ifs.seekg(offset);

		ifs.read((char*)(data.get()), header->data->cb_size);

		return true;
	}

	std::string static GetFileName()
	{
		struct tm t;   //tm结构指针
		time_t now;  //声明time_t类型变量
		time(&now);      //获取系统日期和时间
		localtime_s(&t, &now);   //获取当地日期和时间

		return std::to_string(t.tm_year + 1900) + std::to_string(t.tm_mon + 1) + std::to_string(t.tm_mday) + std::to_string(t.tm_hour);
	}

private:

	// Read wav file header
	bool read_header(const string &filename)
	{
		ifstream ifs(filename, ifstream::binary);
		if (!ifs)
			return false;

		header = make_unique<Wave_header>();

		// Read RIFF chunk
		FOURCC2 FOURCC2;
		ifs.read((char*)&FOURCC2, sizeof(FOURCC2));

		if (FOURCC2 != MakeFOURCC2<'R', 'I', 'F', 'F'>::value) // 判断是不是RIFF
			return false;
		Base_chunk riff_chunk(FOURCC2);
		ifs.read((char*)&riff_chunk.cb_size, sizeof(uint32_t));

		header->riff = make_shared<Base_chunk>(riff_chunk);

		// Read WAVE FOURCC2
		ifs.read((char*)&FOURCC2, sizeof(FOURCC2));
		if (FOURCC2 != MakeFOURCC2<'W', 'A', 'V', 'E'>::value)
			return false;
		header->wave_fcc = FOURCC2;

		// Read format chunk
		ifs.read((char*)&FOURCC2, sizeof(FOURCC2));
		if (FOURCC2 != MakeFOURCC2<'f', 'm', 't', ' '>::value)
			return false;

		Base_chunk fmt_chunk(FOURCC2);
		ifs.read((char*)&fmt_chunk.cb_size, sizeof(uint32_t));

		header->fmt = make_shared<Base_chunk>(fmt_chunk);

		// Read format data
		Wave_format format;
		ifs.read((char*)&format, fmt_chunk.cb_size);

		// Read data chunk
		ifs.read((char*)&FOURCC2, sizeof(FOURCC2));
		if (FOURCC2 != MakeFOURCC2<'d', 'a', 't', 'a'>::value)
			return false;

		Base_chunk data_chunk(FOURCC2);
		ifs.read((char*)&data_chunk.cb_size, sizeof(uint32_t));

		header->data = make_shared<Base_chunk>(data_chunk);

		ifs.close();

		return true;
	}

	bool static checkExistence(const char* filename)
	{
		ifstream Infield(filename);
		return Infield.good();
	}
public:
	shared_ptr<Wave_header> header;
	unique_ptr<uint8_t[]> data;
};

struct PACKET
{
	int ID;
	std::string s;
};

template <typename T>
class MSGqueue
{
	typedef T struct_type;
public:
	MSGqueue() {/* memset(&m_Packet, 0, sizeof(struct_type));*/ m_nSingals = 0; }
	~MSGqueue() {}

	int getsize()
	{
		//std::unique_lock<std::mutex> lock(m_mutex);
		return m_MSGlist.size();
	}
	bool getPacket(struct_type& packet)
	{
		//WaitSignal();
		std::unique_lock<std::mutex> lock(m_mutex);
		if (!m_nSingals)
			m_cv.wait(lock);
		m_nSingals--;
		if (m_MSGlist.size())
		{
			packet = m_MSGlist.front();
			m_MSGlist.pop_front();
			return true;
		}
		else
			return false;
	}
	void putPacket(const struct_type &packet)
	{
		std::unique_lock<std::mutex> lock(m_mutex);
		m_nSingals++;
		m_MSGlist.push_back(packet);
		m_cv.notify_one();
	}

private:
	std::list<struct_type> m_MSGlist;
	//struct_type m_Packet;
	std::mutex m_mutex;
	std::condition_variable m_cv;
	uint16_t m_nSingals;

	bool WaitSignal()
	{
		std::unique_lock<std::mutex> lock(m_mutex);
		if (!m_nSingals)
			m_cv.wait(lock);
		m_nSingals--;
		return true;
	}
};

class Adaptor
{
public:
	Adaptor() 
	{ 
		m_stop = false;
		for (size_t i = 0; i < 65; i++)
		{
			std::string str;
			m_vaudios.push_back(str);
		}
	}
	~Adaptor() { }
	bool setMQsrc(std::shared_ptr< MSGqueue < std::pair< int, std::string > > > pMQ)
	{
		if (pMQ)
		{
			m_pMQ = pMQ;
			return true;
		}
		return false;
	}
	void run()
	{
		while (m_pMQ && !m_stop)
		{
			std::pair<int, std::string > tmp;
			if (m_pMQ->getPacket(tmp))
			{
				m_vaudios[tmp.first].append(tmp.second);
				if (m_vaudios[tmp.first].size() > 1048570)
				{
					CWaveFile::write(std::to_string(tmp.first) + CWaveFile::GetFileName() + ".wav", (char*)m_vaudios[tmp.first].data(), m_vaudios[tmp.first].size());
					m_vaudios[tmp.first].clear();
					int aaa = 0;
				}
			}	
			qDebug() << m_pMQ->getsize() << u8"ID:" << tmp.first ;
		}
		return;
	}
	void setstop(bool stop) { m_stop = stop; }
private:
	std::shared_ptr<MSGqueue<std::pair<int, std::string> > > m_pMQ;
	bool m_stop;
	std::vector<std::string>	m_vaudios;
};

//int main()
//{
//	// Write WAv文件
//	Wave_header header(1, 48000, 16);
//
//	uint32_t length = header.fmt_data->sample_per_sec * 10 * header.fmt_data->bits_per_sample / 8;
//	uint8_t *data = new uint8_t[length];
//
//	memset(data, 0x80, length);
//
//	CWaveFile::write("e:\\test1.wav", header, data, length);
//
//
//	//CWaveFile wave;
//	//wave.read("e:\\test1.wav");
//
//	cout << "end" << endl;
//	getchar();
//	return 0;
//}

